...
class ComparePolymorphisms:
    ...
    def clean_plot_html_data(self, generation_freqs_dict, ref_genome_id):
        '''
        Input1: dictionary of mutation frequencies generated by get_html_generation_frequencies() or get_suspect_html_frequencies()
        Input2: NCBI accession for ref genome
        Output: pandas data frame, cols= position, mutation, generation, frequency
        '''
            ...
            if key[0] == ref_genome_id:
                ...
                for item in enumerate(complete_row):
                    if item[0] in list(generation_dict.keys()):
                        ...
                        freq_by_generation = complete_row[:2] + generation_frequency
                        all_freqs_by_generation.append(freq_by_generation)
        df_html_frequencies_by_generation = pd.DataFrame(all_freqs_by_generation, columns=['position', 'mutation', 'generation', 'frequency'])
        dtype = {'position': str, 'mutation': str, 'generation': int, 'frequency': float}
        for key, value in dtype.items():
            df_html_frequencies_by_generation[key] = df_html_frequencies_by_generation[key].astype(value)
        df_html_frequencies_by_generation['position_mutation'] = df_html_frequencies_by_generation.position + df_html_frequencies_by_generation.mutation
